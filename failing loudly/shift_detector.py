...
#         ind_md_p_vals = np.ones((len(self.dr_techniques), len(self.md_tests))) * (-1)
        ...
        for dr_ind, dr_technique in enumerate(self.dr_techniques):
            ...
            name = DimensionalityReduction(dr_technique).name
            print(name)
            ...
            if name=='PCA':
                tr_red_inverted = shift_reductor_model.inverse_transform(X_tr_red)
                te_red_inverted = shift_reductor_model.inverse_transform(X_te_red)
    
           
                tr_red_inverted = tr_red_inverted.reshape(len(tr_red_inverted),28,28)
                te_red_inverted = te_red_inverted.reshape(len(te_red_inverted),28,28)

                print(np.shape(tr_red_inverted),np.shape(te_red_inverted))
                out_te_red[name] = te_red_inverted
                out_val_red[name] = tr_red_inverted
            
            else:
                print(np.shape(X_te_red.reshape(len(X_te_red),28,28)))
                out_te_red[name] = X_te_red.reshape(len(X_te_red),28,28)
                out_val_red[name] = X_tr_red.reshape(len(X_tr_red),28,28)
...
#         return (od_decs, ind_od_decs, ind_od_p_vals), (md_decs, ind_md_decs, ind_md_p_vals), red_dim, self.red_models, val_acc, te_acc
        ...
        return out_val_red, out_te_red, red_dim, self.red_models, val_acc, te_acc
